...
// ─── scrittura nel blob gestionale (lock ottimistico) ────────────────────────
  ...
  for (let t = 0; t < 6; t++) {
    ...
    for (const v of voci) {
      ...
      dati.presenze[v.data][v.pid] = {
        ...prev, cantiere: v.cantiere, ore: v.ore, oreStraord: v.oreStraord,
        trasferta: v.trasferta, fonte: "telegram", aggiornato: new Date().toISOString(),
      };
    }
